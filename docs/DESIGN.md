...
Carrier-board discrete parts (full BOM in [hardware/pcb.md](hardware/pcb.md)): slide power switch (**SS12D00**, 6 mm handle — the 0.3 A make/break rating is accepted at 4 V, §5), mode button (6×6 mm tactile), 10 kΩ potentiometer (filter-cutoff; **RV097NS** 9 mm PCB-mount, B10K, 5-pin **mono with switch**, right-angle, metal shaft turned directly — no knob; the switch is unused, both ends on GND), 2× 3 mm LED + resistors, battery divider (2× 100 kΩ + 100 nF), 10 kΩ pull-up for GPIO2, bulk electrolytic (≥220 µF) on the battery rail, **2× RCA flying-lead output pad-pairs** (signal + ground; X = PCM5102A L, Y = PCM5102A R) driving the reused ~50 cm RCA cables — series R10/R11 feed the pads, **fitted as 0 Ω wire links** (the DAC module carries its own ~470 Ω output filter, so 100 Ω adds nothing measurable; [hardware/pcb.md](hardware/pcb.md) §3.2 / §10 Q2, closed 2026-07-28), female pin-header sockets for the SuperMini/DAC/TP4056 modules, and a **3-pin pigtail header (VCC / GND / OUT)** for the MAX4466 (on a ~10 cm cable, not a flat on-board footprint).
...
**Scope connection (resolved):** no BNC or TRS on the carrier. The board exposes **2× RCA flying-lead solder pad-pairs** (signal + ground): X = PCM5102A L, Y = PCM5102A R, each fed through its series R10/R11 position — **fitted as a 0 Ω link**, not 100 Ω ([hardware/pcb.md](hardware/pcb.md) §3.2 / §10 Q2, closed 2026-07-28). Signal chain: board RCA pad → reused ~50 cm RCA cable (RCA male, salvaged from the old sigma-delta units) → BNC→RCA adapter already fitted on each scope → scope CH. This frees ~25 mm of board edge and ~€2/board vs the old BNC plan.
...
- [ ] PCM5102A output truly passes DC on the actual purple modules (drive a slow ramp, watch scope) — datasheet says ground-centered/DC-capable. *Measured 2026-07-18:* the module carries a ~470 Ω "471" output filter but is ground-centered (no DC-blocking cap) — confirm the ramp passes DC. **R10/R11 is no longer part of this test:** it was settled as 0 Ω links on 2026-07-28 ([hardware/pcb.md](hardware/pcb.md) §10 Q2), and the ramp could not have decided it anyway — 100 Ω against the module's own 470 Ω shifts amplitude by 0.01 % and phase at 10 kHz by ≲0.07°, identically on both channels.
