...
```
                        ...
                        │                                                               │
 WiFi ─▶ async_udp task ▶ JitterBuffer ──▶ pull_block() ──┐                             │
 (:5000 audio, raw lwIP │  (24576 frames,  deadline       │                             │
  callback, prio 8)     │   512 ms cap,    policy         ▼                             │
      │                 │   60 ms gate)           mode_manager::fill_block ──▶ audio_out::write ──▶ I2S DMA ──▶ PCM5102A ──▶ scope
      ▼                 │                                 ▲                             │
 net_rx task (:5001     │                                 │                             │
 SYNC→timesync + CMD,   │   mic ADC DMA ──▶ mic_in ring ──▶ renderer_local              │
 1 Hz STATUS TX :5002)  │   (24 kHz)        (DC-block)     (X=mic, Y=Butterworth LPF)   │
                        └───────────────────────────────────────────────────────────────┘
```

**Audio RX rework (2026-07-19):** audio packets no longer pass through a socket —
`AsyncUDP` registers a raw lwIP `udp_recv` callback whose handler pushes straight
into the jitter buffer from the `async_udp` task. This bypasses the prebuilt SDK's
6-slot per-socket mailbox (`CONFIG_LWIP_UDP_RECVMBOX_SIZE=6`), which silently
dropped packets during AP bursts. A `lost_packets` counter (missing AUDIO
sequence numbers, `SYNC_PULSE` discontinuities excluded) rides in the STATUS
payload — now 39 B / 59 B on the wire (protocol.md §3.4).
...
| `async_udp` | 8 (bumped from 3 in `on_wifi_up`) | event-driven | audio packet handler: header validation, seq/loss accounting, jitter-buffer push |
| `net_rx` | 5 | 5 ms poll | WiFi connect/reconnect state machine, SYNC → timesync, CMD → mode_manager, 1 Hz STATUS TX |
...
- `JitterBuffer` is strict SPSC (producer `async_udp`, consumer `audio`) with `portMUX` critical sections around index/deadline updates.
...
- Test patterns for bring-up (BOOT button / `pat` console cmd, also settable via `set_pattern` CMD): 100 Hz circle (scope calibration: perfect circle = both channels healthy), 3:2 Lissajous, ramp (DC-deflection go/no-go), square (filter-ringing test) — all phasor-rotation oscillators, no per-sample libm (the C3 has no FPU; per-sample `sinf` hard-wedged the chip once).
