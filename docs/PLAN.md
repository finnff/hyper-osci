# HYPEROSCI — 4-Week Execution Plan

**Today:** 2026-07-17 · **Target:** 2026-08-14 (4 assembled units + rehearsal)
**Canon:** [DESIGN.md](DESIGN.md) — anything here that conflicts with it is a bug in this file.

## Critical path

```
W1: close DESIGN §12 checklist ──▶ W2: KiCad + ORDER PCB ≤ Aug 1 ──▶ W3: fab+ship ──▶ W4: assemble + rehearse
         (blocks layout)              (HARD GATE)                      (dead time,        (no slack after
                                                                        do software)       ~Aug 10 arrival)
```

Everything else (firmware, sync, UNO-Q port) runs in parallel and never blocks the PCB. The one unforgiving dependency chain is: **§12 checklist → layout → order by Aug 1 → boards in hand ~Aug 8–11 → 3–5 days to assemble and rehearse.** JLCPCB standard 2-layer runs are produced in ~24–48 h and DHL to Europe is 3–5 days, but customs can add up to a week with no tracking updates — so Aug 1 is the latest sane order date, not a comfortable one. ([JLCPCB production time](https://jlcpcb.com/help/article/pcb-fabrication-services-and-production-time), [shipping & delivery](https://jlcpcb.com/help/article/shipping-methods-and-delivery-time))
...
- [ ] WiFi RX path: connect to any 2.4 GHz AP, `WiFi.setSleep(false)`, bind port 5000, parse `HYPE_AUDIO` into the jitter buffer; trivial Python sender on the laptop for a first smoke test.
...
- [ ] KiCad schematic: carrier per DESIGN §3/§9 — socketed modules, power path (P-FET + Schottky load sharing per [hardware/pcb.md](hardware/pcb.md)), battery divider, switch, button, pot, LEDs, GPIO2 pull-up, ≥220 µF bulk, 2× scope output connectors.
...
- [ ] **ORDER: 2-layer × 5+ boards at JLCPCB (or PCBWay), DHL express, by Aug 1.** Cost is trivial (~€2–10 board run + ~€15–20 express shipping ⚠️ VERIFY: exact quote at checkout).
- [ ] Laptop **Python streamer stand-in** (this is the seed of the UNO-Q app, see [../src/unoq-controller/README.md](../src/unoq-controller/README.md)): implements [protocol.md](protocol.md) — 988 B `HYPE_AUDIO` packets every 5 ms unicast to each slave on port 5000, `HYPE_SYNC` beacon every 500 ms on 5001, listens for 1 Hz status on 5002 and prints a live stats table (RSSI, buffer depth, drops, underruns, clock offset).
...
- [ ] **4-slave sync test** from the laptop streamer: 30+ min run, packet-loss/underrun stats from status packets, RSSI at 10 m — fully closes DESIGN §12 WiFi robustness item.
...
- [ ] **Port the Python streamer to the UNO-Q** (it's Debian + Python; this should be mostly `scp`). Minimal renderer: built-in test patterns (circle, Lissajous, SVG path playback) — full osci-render integration is explicitly *not* required for Aug 14.
- [ ] 4 slaves on the UNO-Q AP, same stats run as above. Compare against laptop-AP numbers.
...
- [ ] Track PCB shipping daily. **If no tracking movement by Aug 6 → activate Risk 3 plan B now, not on Aug 11.**

## Week 4 — Aug 8–14: assemble, integrate, soak, rehearse

Goal: 4 boxed units + UNO-Q that a stranger could set up from the README.

- [ ] Day boards arrive: assemble **one** PCB, full smoke test (power path first — verify the USB-while-battery-ON rule is actually fixed by the load-sharing circuit before trusting it), then the remaining 3. If boards arrive after Aug 9, the battery test and rehearsal run on the breadboard/protoboard units (Risk 3 posture) and carrier assembly slips past Aug 14 — the PCB never gates the rehearsal.
- [ ] Move all modules from breadboards into sockets; label units 1–4, set slave ids.
- [ ] Battery test: full charge → NETWORK-mode streaming until `VBAT_WARN` (expect ~6.5 h @ 1000 mAh per DESIGN §9); confirm warn/WiFi-off/sleep thresholds fire.
...
- [ ] Buffer days Aug 12–14: fixes only, no new features.
...
| 3 | **Fab/shipping/customs delay eats W4** | No DHL tracking movement by Aug 6, or ETA > Aug 11 | Build 4 units on perma-proto/protoboard in W3–W4 (same sockets, same wiring as breadboard); PCBs become the durable v1 after the deadline. Rehearsal happens either way. |
...
| 5 | **WiFi jitter/loss with 4 unicast streams** (laptop AP or UNO-Q hostapd weak) | W3 stats: sustained >0.5 % loss or recurring underruns at 10 m | In order: pick a clean channel; raise `JB_TARGET_DEPTH_MS` 60 → 100–120 (capacity is 170 ms, latency still < 150 ms); reposition/elevate AP; last resort — dedicated travel router as AP with the controller as a client. |

Runner-up (accepted by design): **UNO-Q app not ready by Aug 14** → the laptop Python streamer *is* the performance-capable fallback controller; UNO-Q polish can slip without endangering the show.
...
| Item | Qty | Notes |
|------|-----|-------|
| BNC connector, right-angle PCB-mount | 10 | 2/board. **Open question (DESIGN §11): BNC vs 3.5 mm TRS.** BNCs are ~€1–2 each and panel-agnostic — recommend ordering BNCs now and 5× TRS jacks (~€0.50) as a cheap hedge. ⚠️ VERIFY: footprint of the exact BNC you buy before W2 layout. |
| Scope cables | — | **8× BNC-BNC cables OR 4× TRS→2×BNC breakouts** — per [FURTHER_CLARIFICATION_NEEDED.md](../FURTHER_CLARIFICATION_NEEDED.md) Q1 answer, decide by ~Jul 21. |
| Female headers, 2.54 mm, fixed sizes: 1×8 / 1×6 / 1×3 / 1×2 | 12 / 6 / 12 / 12 | SuperMini 2× 1×8, PCM5102A 1×6 + 1×3, MAX4466 1×3, TP4056 2× 1×2 — parts for 6 boards, per [hardware/pcb.md](hardware/pcb.md) §5. **Fixed sizes — female strips don't snap cleanly** (each cut destroys a position), so order the exact lengths, not 40-pin strips to cut. |
| 10 kΩ potentiometer, PCB/panel mount, linear | 5 | + knobs. Match footprint to pcb.md. |
| Slide switch, SPDT, THT (power) | 5 | Battery-rail rated ≥ 500 mA. |
...
| JST-PH 2-pin (LiPo) | 5 | Only if batteries/TP4056 wiring needs them — check existing leads. |
| Perma-proto / protoboard | 4 | **Risk 2/3 insurance.** €10 well spent. |

Already owned/ordered (DESIGN §3): 4× ESP32-C3 SuperMini, 4× GY-PCM5102A, 4× MAX4466, 4× TP4056, LiPos, UNO-Q.
