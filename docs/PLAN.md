...
> ✅ **Routing cleared 2026-07-28.** The pre-fab DFM pass moved copper and killed router
> seed 11; a 16-variant re-sweep found exactly one clean result (**seed 33**), now the default.
> The board is **1343 segments, 48 GND vias, 0 unconnected, 0 DRC violations at every
...
|------|-----|--------|-------|
| Scope outputs | — | n/a | **RESOLVED — no BNC/TRS on the board.** 2× RCA output pad-pairs per board (signal + ground: X = PCM5102A L, Y = R, fed through R10/R11 — **fitted as 0 Ω links**, pcb.md §10 Q2). Reuse the existing ~50 cm RCA cables from the old sigma-delta units; scopes are driven via their existing BNC→RCA adapters. **€0 connector spend** — frees ~25 mm of board edge and un-blocks the parts order. |
